...
synth_err synth_track_init(synthTrack *track) {
    ...
    SYNTH_ASSERT(track->notes, SYNTH_MEM_ERR);
    track->buflen = 1;
...
synth_err synth_track_addNote(synthTrack *track, synthNote *note) {
    ...
    if (track->len >= track->buflen) {
        track->buflen *= 2;
        track->notes = realoc(track->notes, track->buflen);
        SYNTH_ASSERT(track->notes, SYNTH_MEM_ERR);
    }
...
void synth_track_synthesize(synthTrack *track, int time) {
    ...
    while (1) {
        ...
        rem = synth_note_synthesize(track->notes[track->pos], rem);
        
        // Stop if there are no samples left
        if (rem == 0)
            break;
        
        // Check if the current note is a loop point and if there is any jump
        if (synth_note_doLoop(track->notes[track->pos]) == SYNTH_TRUE)
            // Go to the loop point and increase the counter
            track->pos = synth_note_jumpToPosition(track->notes[track->pos]);
